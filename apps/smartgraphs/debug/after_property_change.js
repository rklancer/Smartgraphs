...
/*
  ...
  Use this global function to test asynchronous behavior in Sproutcore by waiting to execute tests until  
  desired Sproutcore property reaches a particular value (optionally, simply wait until the value changes.)

  Use within a Qunit test() function like so:
  
  test('after myObject.isLoaded becomes YES, myApp.myData is defined', function () {
  ...
  Optionally, you can skip the third ('value') parameter and run the test callback whenver target.property changes
  (or is notified):
  
  test('after myObject.load is called, its state should transition from LOADING to LOADED', function () {
    myObject.load();
    ...
    afterPropertyChange(myObject, 'state', function () {       // note the lack of the 'value' parameter
      // this assertion is tested as soon as isLoaded is notified, whether via set() or notifyPropertyChange()
  ...
  start() internally and Qunit does not like 
...
(function (undefined) {
  ...
  window.pushStop = function (t) {  
    if (nStops === 0) stop(t);
    nStops++;
  };

  window.popStart = function () {
    --nStops;
    if (nStops === 0) start();
    if (nStops < 0) throw 'afterPropertyChange: popped too many starts';
  };

  window.afterPropertyChange = function (target, property, value, testFn) {
    ...
    // Allow alternate usage where the test proceeds as soon as the target.property changes
